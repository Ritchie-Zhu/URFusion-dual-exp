#!/usr/bin/env bash
# Train ConflictAdd_1. Does not start unless invoked.
set -euo pipefail
...
echo "[$(date '+%F %T')] START ConflictAdd_1 B(f_cap)+a*R(vis,ir,|diff|)"
"$PY" "$CODE/train_fusion_gray.py" \
  --device 0 \
  --experiment ConflictAdd_1 \
  ...
  --cudnn_benchmark \
  "$@" \
  2>&1 | tee "$LOGDIR/ConflictAdd_1.log"

echo "[$(date '+%F %T')] DONE ConflictAdd_1"

"""Train ConflictAdd_1. Outputs stay under our_model_4_ConflictAdd. Do not start unless invoked."""
...
from spatial_moe_loss import compute_conflict_add_loss
...
DEFAULT_CONTENT_ROOT = os.path.join(PROJECT, 'our_model_1_DualMoE', 'train-jobs', 'ckpt')
...
def parse_args():
	parser = argparse.ArgumentParser(description='Train ConflictAdd_1 (leak-free base + gated A2 mixer)')
	parser.add_argument('--device', type=int, default=0)
	parser.add_argument('--experiment', default='ConflictAdd_1')
...
def save_fixed_y_samples(model_F, device, vis_dir, ir_dir, names, out_dir, epoch_display):
	...
	with torch.no_grad():
		for name in names:
			...
			torchvision.utils.save_image(aux['a'], os.path.join(epoch_dir, f'{base}_gate.png'))
			vis_u = aux['f_vis_u'].abs().mean(dim=1, keepdim=True)
			ir_u = aux['f_ir_u'].abs().mean(dim=1, keepdim=True)
			vmax = torch.clamp(torch.maximum(vis_u.max(), ir_u.max()), min=1e-6)
			torchvision.utils.save_image(vis_u / vmax, os.path.join(epoch_dir, f'{base}_vis_u.png'))
			torchvision.utils.save_image(ir_u / vmax, os.path.join(epoch_dir, f'{base}_ir_u.png'))
	model_F.train()
	print(f'  saved {len(names)} fixed Y+gate+unique -> {epoch_dir}', flush=True)
...
def train_one_epoch(loaders, model_F, c_vis_gray, c_ir_gray, vgg16, optimizer, device, args):
	...
	for sample in loaders['train']:
		...
		optimizer.zero_grad()
		y_f, aux = model_F(y_vis, ir_gray)
		stats = compute_conflict_add_loss(
			y_f,
			y_vis,
			ir_gray,
			aux,
...
def main():
	...
	model_F = DualConflictAddNetGray().to(device)
	base_in = int(model_F.moe_block.expert_base.proj.in_channels)
	conflict_in = int(model_F.moe_block.expert_conflict.proj.in_channels)
	if base_in != model_F.feat_channels:
		raise RuntimeError(f'E_base in_channels={base_in}, expected {model_F.feat_channels} (f_cap only)')
	if conflict_in != model_F.feat_channels * 3:
		raise RuntimeError(f'E_conflict in_channels={conflict_in}, expected {model_F.feat_channels * 3}')
	...
	print(f'[START] {experiment} ConflictAdd: B(f_cap) + a * R(vis,ir,|diff|)')
	print(f'  code={UPDATE_CODE}')
	print(f'  data={os.path.abspath(train_dir)}')
	print(f'  C={args.vis_content_ckpt} / {args.ir_content_ckpt} (train-only fusion supervision)')
	print(f'  E_base in_channels={base_in} (f_cap only)')
	print(f'  E_conflict in_channels={conflict_in} (A2 input)')
	print(f'  gate=independent sigmoid, no detach, no L_route, no L_aux')
	print(f'  params={n_params}')
	print(f'  ckpt={os.path.abspath(checkpoint_path)} interval={args.ckpt_interval}')
	print(f'  fixed every {args.save_fixed_every} ep, n={len(fixed_names)}: {fixed_names}')
	print(
		'  HEALTH: OK | GATE_OFF a<0.05 | GATE_ON a>0.95 | GATE_FLAT a_std<0.02 | '
		'UNIQUE_DEAD | BASE_DEAD | RESIDUAL_DEAD res<0.01*e_base'
	)
	print(
		'  STOP AFTER EVAL (not mid-train): 6-col must beat S1 AND A2, '
		'and a must not be GATE_OFF or GATE_ON+GATE_FLAT, else abandon MoE'
	)

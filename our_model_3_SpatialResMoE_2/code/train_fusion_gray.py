"""Train leak-free spatial residual MoE. Outputs stay under our_model_3_SpatialResMoE_2."""
...
def parse_args():
	parser = argparse.ArgumentParser(description='Train leak-free spatial residual MoE')
	parser.add_argument('--device', type=int, default=0)
	parser.add_argument('--experiment', default='SpatialResMoE_2')
...
def main():
	...
	print(f'[START] {experiment} leak-free spatial residual MoE')
	print(f'  code={UPDATE_CODE}')
	print(f'  data={os.path.abspath(train_dir)}')
	print(f'  C={args.vis_content_ckpt} / {args.ir_content_ckpt}')
	print(f'  fusion=E_base(f_cap) + g*E_vis(u_vis)+(1-g)*E_ir(u_ir)')
	print(f'  E_base in_channels={base_in} (f_cap only; no |diff|, no unique)')

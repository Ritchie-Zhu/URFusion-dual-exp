#!/usr/bin/env bash
# B1/B2/B3 fusion eval: Y infer -> VIS chroma colorize -> 25 official metrics (8-proc).
# Appends to existing LLVIP/MSRS/M3FD.xlsx. Does not delete other methods.
...
INFER="$PROJECT/metrics_save/llvip_sample1000/fusion_gray_infer.py"
...
count_images() {
	...
	# LLVIP test_sample1000 uses symlinks; -type f would count 0.
...
for ds in ['LLVIP', 'MSRS', 'M3FD']:
	...
	if keep not in methods:
		raise SystemExit(f'{ds}.xlsx lost {keep}')
	if missing:
		raise SystemExit(f'{ds}.xlsx missing {missing}')
	print(f'  {ds}: ok n={len(methods)} last={methods[-3:]}')
PY
}

log "========== Abl B1-B3 EVAL START =========="

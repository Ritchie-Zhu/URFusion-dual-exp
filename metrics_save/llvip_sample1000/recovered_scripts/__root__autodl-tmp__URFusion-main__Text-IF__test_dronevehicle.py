"""Run Text-IF general fusion inference on DroneVehicle test set."""
...
def parse_args():
	...
	parser.add_argument(
		'--dataset_path',
		type=str,
		default='/root/autodl-tmp/URFusion-main/Text-IF/dataset/DroneVehicle/eval',
	)
	parser.add_argument(
		'--weights_path',
		type=str,
	...
	parser.add_argument(
		'--save_path',
		type=str,
		default='/root/autodl-tmp/URFusion-main/our_model_1_DualMoE/results/Text-IF_DroneVehicle/RGB_fused',
	)
...
def main():
	...
	if not os.path.isfile(args.weights_path) or os.path.getsize(args.weights_path) < 1024:
		raise FileNotFoundError(f'Missing Text-IF weights: {args.weights_path}')
	...
	with torch.no_grad():
		...
		for idx, (vi_path, ir_path) in enumerate(zip(visible_paths, infrared_paths), start=1):
			...
			if idx % 100 == 0 or idx == len(visible_paths):

#!/usr/bin/env bash
# S1 Abl_SpatialResMoE_wo_Res: MSRS infer -> VIS chroma colorize -> official 25-col (run_eval_mp, 8 workers).
# Outputs stay under ablation/Abl_SpatialResMoE_wo_Res. Does not write official metrics_save/*.xlsx.
...
EXP="${EXP:-Abl_SpatialResMoE_wo_Res}"
SHUTDOWN="${SHUTDOWN:-1}"
...
FULL_JSON=$PROJECT/our_model_2_SpatialResMoE/eval_save/SpatialResMoE_3_MSRS/metrics_summary.json
...
lines = [f'MSRS n={n}  Abl_SpatialResMoE_wo_Res vs SpatialResMoE_3 (Full)']

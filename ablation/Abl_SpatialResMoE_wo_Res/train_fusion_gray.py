"""Train Abl_SpatialResMoE_wo_Res: E_base only. Outputs stay under ablation/Abl_SpatialResMoE_wo_Res/train-jobs."""
...
DEFAULT_JOBS = os.path.join(ABLATION_DIR, 'train-jobs')
DEFAULT_CKPT_ROOT = os.path.join(DEFAULT_JOBS, 'ckpt')
DEFAULT_LOG_ROOT = os.path.join(DEFAULT_JOBS, 'metrics_logs')
DEFAULT_FIXED_ROOT = os.path.join(DEFAULT_JOBS, 'fixed_examples')
...
def parse_args():
	parser = argparse.ArgumentParser(description=' SpatialResMoE without residual experts')
	parser.add_argument('--device', type=int, default=0)
	parser.add_argument('--experiment', default='Abl_SpatialResMoE_wo_Res')
	...
	parser.add_argument('--save_fixed_every', type=int, default=0)
...
def save_fixed_y_samples(model_F, device, vis_dir, ir_dir, names, out_dir, epoch_display):
	...
	model_F.train()
	print(f'  saved {len(names)} fixed Y (no gate) -> {epoch_dir}', flush=True)
...
def assert_s1_modules(model_F):
	names = {n for n, _ in model_F.named_modules()}
	banned = [n for n in names if any(k in n for k in ('expert_vis', 'expert_ir', 'router'))]
	if banned:
		raise RuntimeError(f'this ablation must not contain residual experts/router: {banned}')
	if not hasattr(model_F.moe_block, 'expert_base'):
		raise RuntimeError('ablation missing expert_base')
...
def main():
	...
	model_F = DualSpatialBaseOnlyNetGray().to(device)
	assert_s1_modules(model_F)
	...
	if not os.path.isdir(test_ir_dir):
		test_ir_dir = os.path.join(os.path.dirname(test_vis_dir.rstrip(os.sep)), 'IR')
	fixed_names = (
		collect_fixed_names(test_vis_dir, max_n=args.n_fixed) if args.save_fixed_every > 0 else []
	)
	...
	print(f'[START] {experiment} wo residual experts')
	print(f'  code={ABLATION_DIR}')
	print(f'  data={os.path.abspath(train_dir)}')
	print(f'  C={args.vis_content_ckpt} / {args.ir_content_ckpt}')
	print(f'  fusion=E_base(f_cap, |diff|) only; no E_vis/E_ir/router; L_route=0')
	print(f'  lambdas int/grad/branch={args.lambda_int}/{args.lambda_grad}/{args.lambda_branch}')
	print(f'  params={n_params}')
	print(f'  ckpt={os.path.abspath(checkpoint_path)}')
	if args.save_fixed_every > 0:
		print(f'  fixed every {args.save_fixed_every} ep, n={len(fixed_names)}: {fixed_names}')
	else:
		print('  fixed examples disabled')
	print('  HEALTH: OK | UNIQUE_DEAD both unique L1<1e-3 | BASE_DEAD e_base<1e-6')

#!/usr/bin/env bash
# Train A1: Abl_DualMoE_wo_Dual (shared PreNet + MoE). Same protocol as Fusion_noleak_1.
set -euo pipefail
...
ABL=$PROJECT/ablation/Abl_DualMoE_wo_Dual
...
echo "[$(date '+%F %T')] START Abl_DualMoE_wo_Dual"
"$PY" "$ABL/train_fusion_gray.py" \
  --device 0 \
  --experiment Abl_DualMoE_wo_Dual \
  ...
  --cudnn_benchmark \
  "$@" \
  2>&1 | tee "$LOGDIR/Abl_DualMoE_wo_Dual.log"

echo "[$(date '+%F %T')] DONE Abl_DualMoE_wo_Dual"

"""Gray-Y inference for (Abl_DualMoE_wo_Dual / shared PreNet + MoE)."""
...
def parse_args():
	...
	parser.add_argument('--experiment', type=str, default='Abl_DualMoE_wo_Dual')
	parser.add_argument('--ckpt_root', type=str, default=os.path.join(PROJECT, 'our_model_1_DualMoE/train-jobs/ckpt'))
...
def list_pairs(test_dir):
	...
	if not os.path.isdir(vis_dir):
		...
		ir_dir = os.path.join(test_dir, 'IR')
	names = sorted(
		fn for fn in os.listdir(vis_dir)
...
def main():
	...
	out_dir = args.y_out_dir or os.path.join(args.output_root, f'{args.experiment}_{args.dataset}', 'Y_fused')
	os.makedirs(out_dir, exist_ok=True)

	ckpt_path = resolve_latest_ckpt_path(args.ckpt_root, args.experiment)
	ckpt = torch.load(ckpt_path, map_location=device)
	model = SharedPreDualBranchFusionNetGray().to(device)
	model.load_state_dict(ckpt['model_F_state_dict'])
	model.eval()
	if hasattr(model, 'moe_block'):
		model.moe_block.deterministic_inference = True

	print(f'[A1 fusion_gray_infer] {args.experiment} @ {args.dataset}: {len(names)} pairs')

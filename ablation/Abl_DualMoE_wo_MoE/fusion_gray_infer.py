"""Gray-Y inference for (Abl_DualMoE_wo_MoE / single expert)."""
...
def parse_args():
	...
	parser.add_argument('--experiment', type=str, default='Abl_DualMoE_wo_MoE')
...
def main():
	...
	model = DualConvFusionNetGray().to(device)
	model.load_state_dict(ckpt['model_F_state_dict'])
	model.eval()

	print(f'[A2 fusion_gray_infer] {args.experiment} @ {args.dataset}: {len(names)} pairs')

...
LOG=/root/autodl-tmp/URFusion-main/our_model_1_DualMoE/train-jobs/console_logs/Abl_DualMoE_wo_Lbranch.log

echo "[$(date '+%F %T')] RESUME Abl_DualMoE_wo_Lbranch from epoch 58" | tee -a "$LOG"

"$PY" train_fusion_gray.py \
  --device 0 \
  --experiment Abl_DualMoE_wo_Lbranch \
...
echo "[$(date '+%F %T')] DONE Abl_DualMoE_wo_Lbranch resume"

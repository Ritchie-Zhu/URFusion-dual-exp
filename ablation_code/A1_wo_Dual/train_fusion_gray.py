"""Train A1 (Abl_wo_Dual): shared PreNet + MoE. Uses vis-ir-gray/code for data/loss/utils."""
...
BASE_CODE = os.path.abspath(os.path.join(ABLATION_DIR, '..', '..', 'vis-ir-gray', 'code'))
sys.path.insert(0, BASE_CODE)
sys.path.insert(0, ABLATION_DIR)
os.chdir(ABLATION_DIR)

from dataset import M3FDFusionTrain
from dual_shared_fusion import SharedPreDualBranchFusionNetGray
...
def parse_args():
	parser = argparse.ArgumentParser(description='A1: shared PreNet + MoE (Abl_wo_Dual)')
	parser.add_argument('--device', type=int, default=0)
	parser.add_argument('--experiment', default='Abl_wo_Dual')
	parser.add_argument('--fusion_model', default='dual_shared_moe', choices=['dual_shared_moe'])
...
def set_moe_deterministic(model_F, value):
	if hasattr(model_F, 'moe_block') and hasattr(model_F.moe_block, 'deterministic_inference'):
		model_F.moe_block.deterministic_inference = value


def save_fixed_y_samples(model_F, device, vis_dir, ir_dir, names, out_dir, epoch_display):
	os.makedirs(out_dir, exist_ok=True)
	model_F.eval()
	prev_det = getattr(getattr(model_F, 'moe_block', None), 'deterministic_inference', None)
	set_moe_deterministic(model_F, True)
	...
	if prev_det is not None:
		set_moe_deterministic(model_F, prev_det)
	model_F.train()


def build_model(args, device):
	return SharedPreDualBranchFusionNetGray(top_k=args.top_k, noise_epsilon=args.noise_epsilon).to(device)
...
def train_one_epoch(loaders, model_F, c_vis_gray, c_ir_gray, vgg16, optimizer, device, args):
	...
	return {
		...
		'hit_rate_0': mean_hit[0], 'hit_rate_1': mean_hit[1],
		'hit_rate_2': mean_hit[2], 'hit_rate_3': mean_hit[3],
		'importance_0': mean_imp[0], 'importance_1': mean_imp[1],
		'importance_2': mean_imp[2], 'importance_3': mean_imp[3],
...
def main():
	...
	print(f'[START A1] {experiment} | fusion_model={args.fusion_model}')
	print(f'  ablation_code={ABLATION_DIR}')
	print(f'  data={os.path.abspath(train_dir)}')
	print(f'  shared_pre=True (vis_pre/ir_pre merged into shared_pre)')
	...
	for epoch in range(args.numEpoch - begin_epoch):
		...
		print(
			...
			f'L_aux={row["aux_loss"]:.4f} hit=[{hit_list}] '

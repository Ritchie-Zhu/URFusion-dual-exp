"""S1 fairness checks: no residual experts, split identity, grads, L_route=0."""
...
def main():
	...
	print(f'S1 params={n_params(model)} (no vis/ir expert, no router)')
	...
	from s1_loss import compute_s1_loss as _loss
	...
	try:
		sys.path.insert(0, os.path.join(PROJECT, 'vis-ir-gray-moe_update', 'code'))
		...
		print(f'Full params={n_params(full)} > S1; state_dict rejected (fair isolation)')

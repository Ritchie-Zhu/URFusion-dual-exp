#!/usr/bin/env bash
# Train S1 Abl_SR_wo_Res. All outputs under ablation_code/S1_wo_Res/train-jobs.
set -euo pipefail
...
echo "[$(date '+%F %T')] START Abl_SR_wo_Res"
"$PY" "$CODE/train_fusion_gray.py" \
  --device 0 \
  --experiment Abl_SR_wo_Res \
  ...
  --cudnn_benchmark \
  "$@" \
  2>&1 | tee "$LOGDIR/Abl_SR_wo_Res.log"

echo "[$(date '+%F %T')] DONE Abl_SR_wo_Res"

"""Train S1 Abl_SR_wo_Res: E_base only. Outputs stay under ablation_code/S1_wo_Res/train-jobs."""
...
from s1_loss import compute_s1_loss
...
def parse_args():
	parser = argparse.ArgumentParser(description='S1: SpatialResMoE without residual experts')
	parser.add_argument('--device', type=int, default=0)
	parser.add_argument('--experiment', default='Abl_SR_wo_Res')
...
def assert_s1_modules(model_F):
	...
	if banned:
		raise RuntimeError(f'S1 must not contain residual experts/router: {banned}')
	if not hasattr(model_F.moe_block, 'expert_base'):
		raise RuntimeError('S1 missing expert_base')
...
def main():
	...
	print(f'[START] {experiment} S1 wo residual experts')

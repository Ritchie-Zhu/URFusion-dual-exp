"""
Train C_ir_gray on M3FD IR grayscale pairs.

Loss weights match vis-ir/code/train_content_extractor_ir.py (input domain only changed).
C input: [B,1,H,W] IR. VGG auxiliary branch: gray_to_vgg3(IR).
"""
...
from dataset import M3FDIRPairs
from losses.content_extractor_loss import compute_content_extractor_loss_ir
...
def parse_args():
	parser = argparse.ArgumentParser(description='Train gray IR content encoder (C_ir_gray)')
	parser.add_argument('--device', type=int, default=0)
	parser.add_argument('--experiment', default='content_encoder_ir_gray')
	parser.add_argument(
		'--baseDir',
		type=str,
		default='../../vis-ir/dataset/M3FD/',
		help='M3FD root; uses baseDir/train/IR',
	)
...
def train_one_epoch(loaders, model_R, vgg16, optimizer_R, device):
	...
	for sample in loaders['train']:
		...
		if x1.size(1) != 1:
			raise ValueError(f'C_ir_gray expects 1-channel IR, got shape {tuple(x1.shape)}')

		stats = compute_content_extractor_loss_ir(model_R, vgg16, x1, x2)
		stats['loss'].backward()
		optimizer_R.step()
...
def main():
	...
	train_dataset = M3FDIRPairs(train_dir, transform=trans_compose)
	...
	for epoch in range(args.numEpoch - begin_epoch):
		...
		print(
			f'[C_ir_gray] epoch {epoch_display}/{args.numEpoch} '
